...
class SubscriptionScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<int>(
        future: UserRepository().getLeftDaysFreeTrial(),
         builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return CircularProgressIndicator(); // Affiche un écran de chargement pendant l'attente
            }
            if (snapshot.hasError) {
              return Text("error"); // Gérer les erreurs
            }
            if(snapshot.hasData) {
            leftDaysFreeTrial = snapshot.data;
